<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="data" size="small" style="width: 100%;">
      ...
      <el-table-column prop="map.endTime" label="结束时间yyyy-MM-dd HH:mm:ss" />
...
import eForm from './mtactivityform.vue'
export default {
  components: { eForm },
  mixins: [initData],
  data() {
  ...
  methods: {
    ...
    beforeInit() {
      ...
      const sort = 'id,desc'
      this.params = { page: this.page, size: this.size, sort: sort, groupName: 'mshop_meituan_activity_list' }
      return true
    },
    subDelete(id) {
    ...
    edit(data) {
      ...
      _this.form = {
        ...
        activityId: data.map.activityId,
        img: data.map.img,
        title: data.map.title,
        endTime: data.map.endTime,
        sort: data.sort,
        status: data.status
      }
      _this.dialog = true
