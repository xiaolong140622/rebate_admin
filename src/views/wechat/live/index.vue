...
<!--        101：直播中，102：未开始，103 已结束，104 禁播，105：暂停，106：异常，107：已过期-->
        ...
        <el-table-column v-permission="['admin','mwWechatLive:edit','mwWechatLive:del']" label="操作" width="150px" align="center">
          <template slot-scope="scope">
...
<script>
import {  sync } from '@/api/mwWechatLive'
import crudMwWechatLive from '@/api/mwWechatLive'
...
import { addGoods } from '@/api/mwWechatLive'
// crud交由presenter持有
const defaultCrud = CRUD({ optShow: {
    ...
    del: false,
    download: true
  },title: '直播房间', url: 'api/mwWechatLive', sort: 'room_id,desc', crudMethod: { ...crudMwWechatLive }})
...
export default {
  ...
  data() {
    return {
      ...
      syncLoading: false,
      permission: {
        add: ['admin', 'mwWechatLive:add'],
        edit: ['admin', 'mwWechatLive:edit'],
        del: ['admin', 'mwWechatLive:del']
      },
      rules: {
        name: [
