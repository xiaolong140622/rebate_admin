...
import { getInfo, isFormatAttrForActivity } from '@/api/mwStoreProduct'
import { add, edit,getSecKillInfo } from '@/api/mwStoreSeckill'
...
export default {
  ...
  mounted () {
    this.getInfo(),
    initData('api/mwSystemGroupData',{ groupName: 'mshop_seckill_time',status:1 }).then(res => {
  ...
  methods: {
    ...
    addCustomDialog () {
      window.UE.registerUI('mshop', function (editor, uiName) {
        let dialog = new window.UE.ui.Dialog({
          iframeUrl: '/mshop/materia/index',
          editor: editor,
          name: uiName,
          title: '上传图片',
